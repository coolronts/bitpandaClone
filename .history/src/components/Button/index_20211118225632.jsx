...
export function Circle({ icon, color, name, radius=16 }) {
  const styles = {
    container: `flex flex-col text-center  w-${radius+6}`,
    base: `rounded-full h-${radius} w-${radius} flex items-center justify-center shadow-2xl bg-${color}-400 ring-1 ring-gray-900 hover:bg-${color}-500 cursor-pointer`,
  ...
  return (
    <div className={styles.container}>
      ...
      <p className="text-xl font-bold flex-shrink">{name}</p>
    </div>
  )
}
