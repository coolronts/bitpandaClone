...
export default function Tables({
  ...
  const styles = {
    container: "shadow  border-b border-gray-200 rounded-lg py-12",
                ...
                {items.map((item, index) => (
                  ...
                  <tr key={item.id}>
                      ...
                      <SplitScreen containerWeight="flex items-center" leftWeight="flex-shrink" rightWeight="ml-3">
                        ...
                        <div>
                          <p className="text-sm uppercase font-bold">{item.symbol}</p>
                          
                          {(index == lastElement - 1 ? true : false) == true ?
                            <p className="" ref={lastElementRef}>{item.name}</p> :
