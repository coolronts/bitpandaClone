...
export default function TopMoversCard({ detail }) {
  ...
  const priceChange = (price) => {
    setChangePrice((price[price.length - 1][1] - price[0][1]).toFixed(2))
  ...
  return(
    <div className={styles.cardBody}>
      ...
      {changePrice > 0 ?
        <div className={styles.positiveChange}>{changePrice}  {changePercent}% <TiArrowSortedUp /></div> :
        <div className={styles.negativeChange}>{changePrice} {changePercent}% <TiArrowSortedDown/></div>
      }
