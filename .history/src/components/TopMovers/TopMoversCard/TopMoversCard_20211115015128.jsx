...
export default function TopMoversCard({ detail }) {
  ...
  const priceChange = (price) => {
    console.log(price[price.length - 1][1], "price")
    
    setChangePrice((price[price.length - 1][1] - price[0][1]) / price[0][1] * 100)
  ...
  return(
    <div className={styles.cardBody}>
      ...
      <div className={styles.change}>+$1.232 ({changePrice}% <TiArrowSortedUp/>)</div>
    </div>
  )
}