...
export default function TopMoversCard({ detail }) {
  ...
  const priceChange = (price) => {
    setChangePrice(((price[price.length - 1][1] - price[0][1]) / price[0][1] * 100).toFixed(2))
  }
  ...
  return(
    <div className={styles.cardBody}>
      ...
      {changePrice > 0 ?
        <div className={styles.positiveChange}>+$1.232 ({changePrice}% <TiArrowSortedUp />)</div> :
        <div className={styles.negativeChange}>+$1.232 ({changePrice}% <TiArrowSortedDown/>)</div>
      }
