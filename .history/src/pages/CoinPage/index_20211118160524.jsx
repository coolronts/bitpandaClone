...
export default function CoinPage({ coinId }) {
  const Button = tw.button<ButtonProps>`
    text-lg font-bold
    ${(p) => (p.$change > 0 ? "text-green-600" : "text-red-600")}`
  ...
  useEffect(() => {
    coinDetails('bitcoin')
      .then(res => {
        console.log(res, "res")
        console.log(res.market_data.sparkline_7d.price, "market_data.sparkline_7d")
  ...
  const CardContent = () => {
    return (
      <>
        ...
        <div className="text-lg font-bold"> {coin.market_data.market_cap_change_24h_in_currency.nok> 0 && '+'}{coin.market_data.market_cap_change_24h_in_currency.nok} {coin.market_data.market_cap_change_percentage_24h_in_currency.nok> 0 && '+'}{coin.market_data.market_cap_change_percentage_24h_in_currency.nok}  </div>
