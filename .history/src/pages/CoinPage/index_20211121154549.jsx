...
export default function CoinPage() {
  ...
  const { coinId } = useParams();
    const ChartContent = () => {
      ...
      function marketChange(data) {
        setPriceChange((data[data.length - 1][1] - data[0][1]).toFixed(2))
        setPercentageChange(((data[data.length - 1][1] - data[0][1])*100/(data[0][1])).toFixed(2))
      }
      ...
      useEffect(() => {
        ChartData(coinId, dayChart).then(res => {
    ...
    return (
      <div className={styles.body}>
        ...
        <div className={`${styles.priceChange} ${priceChange > 0 ? "text-green-500" : "text-red-500"}`}>
          {priceChange > 0 && '+'}
          {priceChange} /
          {percentageChange > 0 ? ' +' : ' '}
          {percentageChange + '%'}
          ...
          <div>
            <section className={styles.body}>
              <SplitScreen leftWeight={'w-full mr-12'} rightWeight={'w-1/2'}>
                <div id="left">
                  <SplitScreen containerWeight="flex mb-4" leftWeight={'flex-shrink mr-2'} rightWeight={'w-1/3'}>
