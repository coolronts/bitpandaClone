...
export async function currentPrice(id) {
    const response = await axios.get(
        `${baseLink}simple/price?ids=${id}&vs_currencies=nok&include_market_cap=true&include_24hr_vol=true&include_24hr_change=tru`
    );
...
export async function dayChartData(id) {
    const chartData = await axios.get(
        `${baseLink}coins/${id}/market_chart?vs_currency=nok&days=1&interval=hourly`
    );
